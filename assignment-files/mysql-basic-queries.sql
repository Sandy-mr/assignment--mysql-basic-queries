...
SELECT FIRST_NAME, LAST_NAME, SALARY FROM Employees WHERE (SALARY > 10000 AND SALARY <15000 ) AND (DEPARTMENT_ID = 30 OR DEPARTMENT_ID = 100);
...
SELECT LAST_NAME, JOB_ID, SALARY FROM Employees WHERE (JOB_ID = 'IT_PROG' OR JOB_ID = 'SH_CLERK') AND (SALARY != 4500 OR SALARY != 10000 OR SALARY != 15000);
...
SELECT FIRST_NAME, LAST_NAME, HIRE_DATE FROM Employees WHERE HIRE_DATE > '1987-07-01';
...
SELECT FIRST_NAME FROM Employees WHERE CHAR_LENGTH(LAST_NAME) > 6;

# 10 Write a query to display the last name of employees having 'e'
#    as the third character.
#     [HINT: look up SUBSTRING operator ]

SELECT LAST_NAME FROM Employees WHERE SUBSTRING(LAST_NAME, 3) LIKE 'e%';
